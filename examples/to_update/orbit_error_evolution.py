...
for ps_lst in passes[0]:
    ...
    track = sorts.controller_v1.Tracker(
        radar=radar, t=t[ps.inds[use_inds]], ecefs=states[:3, ps.inds[use_inds]]
    )
...
ax.semilogy(
    t_diff / 3600.0,
    r_diff_stdev_all * 1e-3,
    label="Orbit Determination Covariance (ODC)",
    color="C2",
)
ax.semilogy(
    t_diff / 3600.0, r_diff_stdev_all_drag * 1e-3, label="ODC + Drag uncertainty", color="C1"
)
