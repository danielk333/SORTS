...
def process_tex(lines):
    ...
    new_lines = []
    for line in lines:
        if (line.startswith(r'\section{sorts.')
            or line.startswith(r'\subsection{sorts.')
            or line.startswith(r'\subsubsection{sorts.')
            or line.startswith(r'\paragraph{sorts.')
            or line.startswith(r'\subparagraph{sorts.')
            ):
            pass # skip!
        else:
