...
class Scanning(Simulation):
    ...
    def plot(self, txi=0, rxi=0):
        ...
        for radar, meta in scan_sched(np.arange(0, scan.cycle(), scan.dwell())):
            for tx in radar.tx:
                point_tx = (
                    tx.pointing_ecef / np.linalg.norm(tx.pointing_ecef, axis=0) * scan_sched.r.max()
                    + tx.ecef
                )
                axes[0][0].plot(
                ...
                for rx in radar.rx:
                    ...
                    for ri in range(pecef.shape[1]):
                        point_tx = (
                            tx.pointing_ecef
                            / np.linalg.norm(tx.pointing_ecef, axis=0)
                            * scan_sched.r[ri]
                            + tx.ecef
                        )
        ...
        for ind in range(len(objs)):
            for pi in range(len(self.passes[ind][txi][rxi])):
                ...
                axes[0][0].plot(
                    self.states[ind][0, ps.inds],
                    self.states[ind][1, ps.inds],
                    self.states[ind][2, ps.inds],
                    "-",
                )

                if dat is not None:
                    axes[0][1].plot(
                        dat["t"] / 3600.0, dat["range"] * 1e-3, "-", label=f"obj{ind}-pass{pi}"
                    )
