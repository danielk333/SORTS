...
# load saved data if there is a save, otherwise generate and save them
datas = []
passes = []
states = []
pickle_fpath = (
...
if os.path.isfile(pickle_fpath):
    ...
    with open(pickle_fpath, "wb") as f:
        pickle.dump({"datas": datas, "passes": passes, "states": states}, f)
...
radar_meta_pairs = list(scanner_ctrl(np.arange(0, scan.cycle(), scan.dwell())))
for radar, meta in radar_meta_pairs:
    for tx in radar.tx:
        point_tx = (
