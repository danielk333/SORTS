...
class StxMrxSimulation:
    ...
    def __init__(
        ...
        self.save_fpath = save_fpath if isinstance(save_fpath, Path) else Path(save_fpath)
        ...
        # create a save file on init
        self.save(save_fpath)

    @classmethod
    def from_controllers(
        cls,
        controllers: t.Sequence[controller.ControllerBase],
        schedule: scheduling.ScheduleDb | scheduling.ScheduleDataframe,
        epoch: Datetime_Like,
        start_time: Datetime_Like,
        end_time: Datetime_Like,
        space_objects: t.Sequence[sorts.SpaceObject],
        ...
        if isinstance(schedule, pd.DataFrame):  # if it is a `scheduling.ScheduleDataframe`
            # TODO: creating an internal `ScheduleDb` is just a tmp solution to maintain the public api
            schedule_db = scheduling.ScheduleDb.empty(save_fpath)
            schedule_db.add_dataframe(schedule, "_combined_schedule")
        else:
    ...
    @classmethod
    def load(cls, fpath: str | Path) -> t.Self:
        """
        Reconstruct itself from a save file generated by the `save` method.

        Args:
            fpath: The path to the sqlite3 database.
        """

        if isinstance(fpath, str):
            fpath = Path(fpath)

        db_conn = sqlite3.connect(fpath, autocommit=True, timeout=15.0)

        db_conn.execute("PRAGMA foreign_keys = ON")
        db_conn.execute("PRAGMA journal_mode = WAL")

        blob = db_conn.execute(
            f"""SELECT blob FROM "{cls.save_table_name}" LIMIT 1""",
        ).fetchone()[0]
        data = pickle.loads(blob)

        instance = cls(
            station_map=data["station_map"],
            station_id_pairs=data["station_id_pairs"],
            schedule_db=scheduling.ScheduleDb(
                db=scheduling.ScheduleDbConnection(db_conn),
                dataframe_names=data["schedule_db__dataframe_names"],
            ),
            exp_detail_map=data["exp_detail_map"],
            epoch=data["epoch"],
            start_time=data["start_time"],
            end_time=data["end_time"],
            space_objects=data["space_objects"],
            interpolated_propagations=data["interpolated_propagations"],
            save_fpath=data["save_fpath"],
            progress=data["progress"],
            passages=data["passages"],
        )

        # TODO: include them in init as well?
        instance.sim_units = data["sim_units"]
        instance.obss = data["obss"]

        return instance

    def save(self, fpath: str | Path):
        """
        Save itself inside a sqlite3 database under the table `self.save_table_name`.
        Use the `load` classmethod to reconstruct itself from a save file.
        ...
        blob = pickle.dumps(
            {
                ...
                "schedule_db__dataframe_names": self.schedule_db.dataframe_names,
                "exp_detail_map": self.exp_detail_map,
                "epoch": self.epoch,
                "start_time": self.start_time,
                "end_time": self.end_time,
                "space_objects": self.space_objects,
                "interpolated_propagations": self.interpolated_propagations,
                "save_fpath": self.save_fpath,
        ...
        db_conn.execute(f"""DROP TABLE IF EXISTS "{self.save_table_name}";""")
        db_conn.execute(
            f"""
            CREATE TABLE "{self.save_table_name}" (
        ...
        db_conn.execute(f"""INSERT INTO "{self.save_table_name}" (blob) VALUES (?)""", [blob])
        db_conn.commit()
