...
{
	scheduler_python_functions_callback.get_pointing_direction = _get_pointing_direction;
	scheduler_python_functions_callback.get_n_dirs_per_time_slice = _get_n_dirs_per_time_slice;
	scheduler_python_functions_callback.save_pointing_direction_arrays = _save_pointing_direction_arrays;
}
...
// 			{
// 				n_dirs_per_tslice = scheduler_python_functions_callback.get_n_dirs_per_time_slice(txi, rxi, ti, 0);
...
// 				scheduler_python_functions_callback.get_pointing_direction(txi, rxi, ti, pointing_directions_tmp, 0);
...
// 		for(int ti = 0; ti < _n_time_points; ti++)
// 		{
// 			n_dirs_per_tslice = scheduler_python_functions_callback.get_n_dirs_per_time_slice(txi, 0, ti, 1);

// 			pointing_directions_tmp = (double*)realloc(pointing_directions_tmp, n_dirs_per_tslice*3*sizeof(double));
// 			scheduler_python_functions_callback.get_pointing_direction(txi, 0, ti, pointing_directions_tmp, 1);
...
// 	for(int i = 0; i < 3; i++)
// 		scheduler_python_functions_callback.save_pointing_direction_arrays(pointing_directions_tx[i], pointing_directions_rx[i], n_points_tx, n_points_rx, i);
