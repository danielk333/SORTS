...
from sorts.detection_config.detection_config import (
    DetectionConfigProtocol,
    find_simultaneous_passes_time_ranges,
)


# TODO: separate into `Param` dataclass and behavioural class that implement `DetectionConfigProtocol`?
...
def calculate_observation(
    ...
    if schedule_mask is not None:
        ...
        tx_schedule = schedule.filter_schedule_by_mask(dcfg.tx_schedule, schedule_mask)
        rx_schedule = schedule.filter_schedule_by_mask(dcfg.rx_schedule, schedule_mask)
    ...
    # )  # TODO: chk if needed
    powers = np.array([dcfg.exp_num_map[n].power for n in tx_schedule.exp_num], dtype=np.float64)
    bandwidths = np.array(
