...
from sorts.targets.propagator.kepler import *
from sorts.radar.system.instances.mock import gen_mock
...
class TestSimulateScan(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.T = np.pi*2.0*np.sqrt(7500e3**3/constants.MU_earth)
        ...
        self.rise_ang = 90.0 - np.degrees(np.arccos(constants.wgs84_a/7500e3))
        ...
        self.full_t = np.linspace(0, self.T, num=self.num)
    ...
    def test_get_detections(self):
        ...
        best = np.argmin(det_times[0]['on_axis_angle'])

        self.assertLess(np.abs(det_times[0]['tm'][best] - self.T*0.25), 2.0) #best detection is overhead and should be after quater orbit, 2sec tol
        self.assertLess(np.abs(det_times[0]['range'][best] - (7500e3 - wgs84_a)), 1.0) #range is knownish, 1 meters res

        self.assertLess(np.abs(det_times[0]['t0'][0] - self.rise_T), self.T/self.num*20.0)
        self.assertLess(np.abs(det_times[0]['t1'][0] - self.fall_T), self.T/self.num*20.0)

        self.assertLess(np.min(det_times[0]['on_axis_angle']), 0.1) #less then 1/10 deg
