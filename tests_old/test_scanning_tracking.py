...
class TestSimulateScan(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.num = simulate_tracking.find_linspace_num(
            t0=0.0, t1=self.T, a=7500e3, e=0.0, max_dpos=1e3
        )
    ...
    def test_get_detections(self):
        ...
        self.assertLess(
            n.abs(det_times[0]["tm"][best] - self.T * 0.25), 2.0
        )  # best detection is overhead and should be after quater orbit, 2sec tol
        self.assertLess(
            n.abs(det_times[0]["range"][best] - (7500e3 - wgs84_a)), 1.0
        )  # range is knownish, 1 meters res
