...
track = sorts.controller_v1.Tracker(radar=radar, t0=0, t=t[inds], dwell=0.1, ecefs=states[:3, inds])
...
print(
    f"G @ F_1 vs F_2 = {radar.tx[0].beam.gain(k, ind=(0,0))} vs {radar.tx[0].beam.gain(k, ind=(0,1))} "
)
print(
    f"G @ F_1 vs F_2 = {radar.tx[0].beam.gain(k, ind=dict(frequency=0))} vs {radar.tx[0].beam.gain(k, ind=dict(frequency=1))} "
)
...
class Tracking(sorts.scheduler.StaticList, sorts.scheduler.ObservedParameters):
    ...
    def get_beam_gain_and_wavelength(self, beam, enu, meta):
        ...
        if "frequency" in beam.parameters:
            ...
            if size is not None:
                g = [beam.gain(enu, ind={"frequency": pi}) for pi in range(size)]
                ind = np.argmax(g)
...
axes[1][1].plot(
    (data[0][0][0]["t"] - ps.start()) / 3600.0,
    10 * np.log10(data[0][0][0]["snr"]),
    ".",
    label="observed-pass",
)
