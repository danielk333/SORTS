...
class ObservedParameters(Scheduler):
    ...
    **Parameters calculated**
    ...
    def calculate_observation_jacobian(
        self, txrx_pass, space_object, variables, deltas, transforms={}, **kwargs
    ):
    ...
    def get_beam_gain_and_wavelength(self, beam, enu, meta):
        ...
        if len(beam.pointing.shape) > 1:
            g = np.max(
                [beam.gain(enu, ind={"pointing": pi}) for pi in range(beam.pointing.shape[1])]
            )
        else:
            g = beam.gain(enu)
    ...
    def vectorized_get_beam_gain_and_wavelength(
        self, t, vectorized_data, tx_enus, rx_enus, tx_index, rx_index
    ):
    ...
    def vectorized_observable_filter(
        self, t, vectorized_data, tx_enus, rx_enus, tx_index, rx_index
    ):
        ...
        if vectorize:
            ...
            if calculate_snr:
                ...
                (
                    ...
                    rx_g,
                    rx_wavelength,
                ) = self.vectorized_get_beam_gain_and_wavelength(
                    t[keep],
                ...
                if blind_ranges:
                    ...
                    for ch_txi, ch_rxi in self.radar.joint_stations:
                        if ch_rxi == rxi:
                            ...
                            snr_modulation[keep][inds] = (
                                ipps[keep][inds] - ipp_f[inds]
                            ) / pulse_lengths[keep][inds]
                        ...
                        for ch_txi, ch_rxi in radar.joint_stations:
                            if ch_rxi == rxi:
                                ...
                                if ipp_f <= radar.tx[txi].pulse_length:
                                    ...
                                    snr_modulation = (radar.tx[txi].ipp - ipp_f) / radar.tx[
                                        txi
                                    ].pulse_length
                                break
