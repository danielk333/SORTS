...
class ObservedTracking(Tracking, ObservedParameters):
    ...
    def set_measurements(self):
        ...
        for ind, so in enumerate(self.space_objects):
            ...
            logger.info(
                f"Propagating {sum(len(t) for t in t_vec)} measurement states for object {ind}"
            )
    ...
    def generate_schedule(self, t, generator, group_target=False):
        data = np.empty(
            (len(t), len(self.radar.tx) * 2 + len(self.radar.rx) * 3 + 1), dtype=np.float64
        )
...
for key in grouped_data:
    pass_id = np.array(
        [x[-1] for x in grouped_data[key]], dtype=np.int
    )  # pass index is last variable
    ...
    for pi, ps in enumerate(passes):
        ax = sorts.plotting.local_tracking(
            ...
            add_track=pi
            > 0,  # if there are more then one, dont redraw all the extra, just add the track
        )
    ax.set_title(key)
...
rx_head += [
    f"RX{i}-{co}" for i in range(len(e3d_demo.rx)) for co in ["az [deg]", "el [deg]", "r [km]"]
]
