...
from sorts.controller import FenceScanController
...
class MpiExample(sorts.MpiQueuedExecution):
    ...
    def master_process(self):
        ...
        scan_min_elevation = 30
        beam_w = tx_station.beam_parameters.beam_width_scaling
        half_power_angle = np.degrees(
            beam_w * tx_station.wavelength / (tx_station.beam_parameters.radius * 2)
        )
        ...
        fence_scan_controller = FenceScanController.from_scan_spec(
            ...
            min_elevation=scan_min_elevation,
            pointings_per_cycle=int(2 * (90 - scan_min_elevation) / half_power_angle),
            scan_range=np.linspace(300e3, 1200e3, num=100),
        )

        scanning_schedule = fence_scan_controller.generate(start_time, end_time)
        exp_id_stn_id_pairs_map = fence_scan_controller.get_experiment_id_station_id_pairs_map()

        master_sch = priority_scheduling([scanning_schedule], exp_id_stn_id_pairs_map)
        ...
        for spobj_id in sim_unit_fpath_grp_by_pert:
            ...
            for fpath in sim_unit_fpath_grp_by_pert[spobj_id][0]:
                ...
                with open(fpath, "rb") as f:
                    ...
                    true_sim_unit_multistatic_set.append(sim_unit)
                for obs in sim_unit.observations:
                    state = obs.get_state_slice()
                    detected_indecies = state[_SuK.snr].to_numpy() > 10**1.2
                    obj_detections.append(state[{_SuK.multi_index: detected_indecies}])
                obj_detections = xr.concat(obj_detections, dim=_SuK.multi_index)
                print(obj_detections)
                det_out = fpath.parent / "detections_xarr.pickle"
                with open(det_out, "wb") as f:
                    sim_unit: SimulationUnit = pickle.dump(obj_detections, f)
