...
import sorts
from sorts import controller_v1 as ctrl
        ...
        for ind, mrad in zip(range(len(tt)), rc(tt)):
            ...
            if ind < 2:
                nt.assert_almost_equal(
                    radar.tx[0].beam.pointing, np.array([0, 0, 1], dtype=np.float64), decimal=6
                )
            else:
                nt.assert_almost_equal(
                    radar.tx[0].beam.pointing, np.array([0, 1, 0], dtype=np.float64), decimal=6
                )
