    ...
    def __init__(
        ...
        for dt in self.dtypes:
            if np.dtype(dt).char == "U":
                raise TypeError(
                    "Initialized Population cannot use the save function with Unicode [U] numpy strings, try using ASCII [S] strings instead."
                )
    ...
    def __getitem__(self, key):
        if isinstance(key, tuple):
            if len(key) == 2:
                ...
                if is_slice:
                    tmp_data = np.empty(
                        (self.data.shape[0], len(self.fields)), dtype=np.dtype(self._default_dtype)
                    )
            ...
            if key < self.data.shape[0]:
                ...
                raise Exception(
                    "Row number {} outside range of population {}".format(key, self.data.shape[0])
                )
        else:
