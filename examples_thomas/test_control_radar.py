...
final_control_sequence_plt = scheduler.run(controls_plt)

figs = plotting.plot_scheduler_control_sequence(controls, final_control_sequence_plt, scheduler, logger=logger, profiler=p)
...
print("radar_states, ", radar_states)

fmts = ["b-", "m-", "k-", "-c"]
for period_id in range(len(final_control_sequence["t"])):
    ax = plotting.plot_beam_directions(radar_states["pointing_direction"][period_id], eiscat3d, ax=ax, linewidth_rx=0.08, linewidth_tx=0.08, alpha=0.8)
