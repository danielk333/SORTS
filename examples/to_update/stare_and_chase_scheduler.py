    ...
    def update(self, space_object, start_track=None):
        ...
        if self.tracking_object is not None:
            ...
            if start_track is None:
                ...
                self.states_t = np.arange(
                    start_track, start_track + self.max_predict_time, self.timeslice
                )
        ...
        self.scanner = sorts.controller_v1.Scanner(
            self.radar,
            self.scan,
        ...
        if self.tracking_object is not None:
            ...
            self.tracker = sorts.controller_v1.Tracker(
                radar=self.radar,
            ...
            self.scanner.t = self.scanner.t[
                np.logical_or(
                    self.scanner.t < self.tracker.t.min(), self.scanner.t > self.tracker.t.max()
                )
            ]
